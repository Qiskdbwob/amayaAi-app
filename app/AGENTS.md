...
- `src/main/java/com/amaya/intelligence/data/local/`: local storage and database layer, including Room, file-backed session/skill stores, and stable workspace-memory UUID metadata/remapping.
...
- `src/main/java/com/amaya/intelligence/data/repository/`: repository layer that orchestrates AI, personas, files, conversations, active/superseded user/workspace memory, workspace-scoped recall, evidence-backed skills, pending proposals, and maintenance. `About You` writes only through the explicit memory tool; plain chat, reflection, and pending proposals cannot write it. Daily logs, global catch-all memory, model-owned importance, and memory archive/delete/restore are intentionally absent.
