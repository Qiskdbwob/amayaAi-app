...
- `src/main/java/com/amaya/intelligence/impl/local/browser/`: GeckoView controller, runtime/WebExtension bridge, session manager, DOM inspection, and safety guard. Browser task results are enriched with `site_memory` — saved workspace facts for the active site host — so the agent reuses previously learned site knowledge (login flows, selectors, patterns).
...
- `src/main/java/com/amaya/intelligence/domain/security/CommandValidator.kt`: shell/path validation with a blacklist-based non-destructive command classifier: trusted patterns always run, and when `TerminalSettings.autoApproveNonDestructive` is enabled (default on, toggle in the Terminal settings screen) any non-destructive command not in Trusted Commands (read-only commands, MCP invocations, builds, scripts) is auto-approved — only destructive commands (deletion, overwrite, permission/privilege changes, process control, effectful git subcommands) require review. Host-enforced workspace containment: `validateToolCall(…, workspacePath)` denies shell commands that reference absolute paths outside the active workspace or `cd` out of it, even for trusted patterns, so the AI never leaves the workspace through the shell.
...
- `src/main/java/com/amaya/intelligence/data/repository/SelfImprovementPipeline.kt`: post-turn self-learning — skill proposals from verified successful workflows, recovery-patch proposals after repeated failures, failure-lesson proposals when the same workflow fails repeatedly with the same error, auto-consolidation proposals for durable user/workspace facts from explicit user markers, and site-scoped browser learning (browser workflows are fingerprinted per site host so pitfalls/skills are learned per site) — all user-approval gated.
...
- `src/main/java/com/amaya/intelligence/data/repository/`: repository layer that orchestrates AI, files, signed GitHub APK updates, Chat/Project/Agent-group-owned conversations, persistent target-agent delegation turns, bounded owner-scoped recall, imported reference documents, active/superseded user/workspace memory (local field-weighted recall scoring: title/attribute/content + bigram co-occurrence + recency decay + EN/ID synonyms; hosts mentioned in the user message boost site-specific recall), evidence-backed skills, pending proposals, terminal wildcard policy, and maintenance. Persona auto-save and self-improvement auto-save modes are absent; all proposals (skills, site pitfalls, user/workspace facts) remain approval-gated. Daily logs, global catch-all memory, model-owned importance, and memory archive/delete/restore are intentionally absent.
