...
class AiSettingsManager @Inject constructor(
    ...
    val settingsFlow: Flow<AiSettings> = context.dataStore.data.map { prefs ->
        ...
        AiSettings(
            ...
            autoApproveTools = prefs[KEY_AUTO_APPROVE_TOOLS] ?: false
        )
    }
    ...
    suspend fun setAutoApproveTools(enabled: Boolean) {
        context.dataStore.edit { it[KEY_AUTO_APPROVE_TOOLS] = enabled }
    }
...
data class AiSettings(
    ...
    /** When true, non-dangerous tool confirmations are auto-approved. */
